...
#Importación   --------------------------------------------------------
data <- read_sheet("1qtyC5cpkb1O8TCjYQ-_IaIHbHR6djy84aJoZd_Vgf4o")
...
principal <- principal %>% 
  left_join(select(data,ref,eval,fecha,`Submission ID`, matches("68"):matches("73")),by=join_by(user_pass==ref))
...
principal <- principal %>% 
  rename(genero = matches("68"),  # buscando por número de pregunta
         generacion = matches("69"),
         jerarquia = matches("70"),
         tiempo_laboral = matches("71"),) 
