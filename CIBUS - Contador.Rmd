...
#Importación   --------------------------------------------------------
data <- read_sheet("1Fz9gIQrvjMQCt_ybzJPQgXAP3rD8_BX_rjTUzanVneE")
...
principal <- principal %>% 
  left_join(select(data,ref,eval,fecha,`Submission ID`, matches("1."):matches("7.")),by=join_by(user_pass==ref))
...
tema_general_hc <- hc_theme_merge(
  hc_theme_elementary(),
  hc_theme(
    colors = c("#14213D","#29BEB4","#FCA311","#298DB4","#C3EC95"),
    chart = list(
      backgroundColor = "#FFF",
      style = list(
          fontFamily = "Roboto",
...
```{r Avance por Empresa}

contador_empresa <- principal %>% 
  filter(!is.na(company)) %>% # Evitar vacios
...
layout_column_wrap(
  ...
  card_body(
   contador_empresa %>% 
    hchart("bar", hcaes(x = company, y = total_prop, z = total_respuestas), name = "Población", showInLegend = TRUE,
           borderRadius = 10,
           dataLabels=list(enabled=TRUE, format="{point.y:.0f}% ({point.z:.0f})")) %>%
    hc_add_series(type = "bar",data = contador_empresa,hcaes(x = company, y = prop_Completado, z = respuestas_Completado),
    ...
    hc_title(text = "Avance de Respuestas por Empresa") %>%
...
contador_lider <- principal %>% 
  filter(!is.na(lider1)) %>% # Evitar vacios
  group_by(company,lider1,estado) %>% 
  summarise(respuestas = n()) %>% 
  mutate(prop = round(respuestas/sum(respuestas)*100,0)) %>%
  pivot_wider(names_from = estado, values_from = c(respuestas,prop), values_fill = 0) %>% 
  mutate(total_respuestas = rowSums(across(starts_with("respuestas"))),
         total_prop = rowSums(across(starts_with("prop"))),
         titulo = paste(company," - ",lider1, sep = "")) 
...
layout_column_wrap(
  ...
  card_header(
    tooltip(
      bs_icon("info-circle"),
      "Para visualizar los líderes completamente, hacer zoom en el gráfico seleccionando con el cursor un espacio dentro de las barras y utilizar la barra de desplazamiento lateral."
    )),
  card_body(
   contador_lider %>% 
    hchart("bar", hcaes(x = titulo, y = total_prop, z = total_respuestas), name = "Poblacion", showInLegend = TRUE,
           borderRadius = 10,
           dataLabels=list(enabled=TRUE, format="{point.y:.0f}% ({point.z:.0f})")) %>%
    hc_add_series(type = "bar",data = contador_lider,hcaes(x = titulo, y = prop_Completado, z = respuestas_Completado), 
                  name = "Completado", color = "#29BEB4",showInLegend = TRUE,
                  borderRadius = 10,
                  dataLabels=list(enabled=TRUE, format="<span style='color:{series.color}'>{point.y:.0f}% ({point.z:.0f})</span>")) %>%
